...
for f in "$CARBON_HOME"/lib/xboot/*.jar; do
...
for f in "$CARBON_HOME"/bin/*.jar; do
  if [ "$f" != "$CARBON_HOME/bin/*.jar" ]; then
    CARBON_CLASSPATH="$CARBON_CLASSPATH":$f
  fi
done
for t in "$CARBON_HOME"/lib/commons-lang*.jar; do
...
while [ "$status" = "$START_EXIT_STATUS" ]; do
  $JAVACMD \
